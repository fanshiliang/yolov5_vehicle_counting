...
def main():
	...
	y_path = r'yolo-coco'
	g1 = r".\images\Demo_Button_States.png"
	sg.theme('LightGreen')
	left_col = [
			...
			[sg.Text('Line1'), sg.In(y_path,size=(40,1), key='yolo'), sg.FolderBrowse()],
			[sg.Text('Line2'), sg.Slider(range=(0,1),orientation='h', resolution=.1, default_value=.5, size=(15,15), key='confidence')],
			[sg.Text('Line3'), sg.Slider(range=(0,1), orientation='h', resolution=.1, default_value=.3, size=(15,15), key='threshold')],
			[sg.OK(), sg.Cancel()]
				]
	right_col = [
      	[sg.Image(filename=g1, key='image', size=(960,540))],
    ]
	...
	win = sg.Window('汽车流量统计系统', layout, finalize=True,  resizable=True, use_default_focus=False, right_click_menu=sg.MENU_RIGHT_CLICK_EDITME_VER_EXIT)
